-- Drop and recreate get_database_schema function with fixed queries
...
BEGIN
  ...
  -- Get all tables with their columns (using separate query)
  WITH table_list AS (
    SELECT table_name
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'policy_cmd', polcmd::text,
      'policy_roles', polroles::text[],
      'policy_qual', pg_get_expr(polqual, polrelid),
      'policy_with_check', pg_get_expr(polwithcheck, polrelid)
    )
  ) INTO policies
  FROM pg_policies
...
COMMENT ON FUNCTION public.get_database_schema() IS 'Extract complete database schema for backup generation - fixed nested aggregate issue';
